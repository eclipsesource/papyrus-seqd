...
public class UMLNewDiagramFileWizard extends Wizard {
	...
	public UMLNewDiagramFileWizard(URI domainModelURI, EObject diagramRoot, TransactionalEditingDomain editingDomain) {
		...
		myFileCreationPage.setDescription(
				NLS.bind(Messages.UMLNewDiagramFileWizard_CreationPageDescription, RepresentationKind.MODEL_ID));
		IPath filePath;
		...
		myFileCreationPage
				.setFileName(UMLDiagramEditorUtil.getUniqueFileName(filePath, fileName, "PapyrusUMLSequence_diagram")); //$NON-NLS-1$
	...
	public boolean performFinish() {
		...
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(myEditingDomain,
			...
			protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info)
				...
				String diagramVID = VisualIDRegistry
						.getDiagramVisualID(diagramRootElementSelectionPage.getModelElement());
				if (diagramVID.equals(RepresentationKind.DIAGRAM_ID)) {
					return CommandResult.newErrorCommandResult(Messages.UMLNewDiagramFileWizard_IncorrectRootError);
				}
				Diagram diagram = ViewService.createDiagram(diagramRootElementSelectionPage.getModelElement(),
						RepresentationKind.MODEL_ID, Activator.DIAGRAM_PREFERENCES_HINT);
		...
		try {
			...
			Activator.log.error("Unable to create model and diagram", e); //$NON-NLS-1$
		} catch (IOException ex) {
			Activator.log.error("Save operation failed for: " + diagramModelURI, ex); //$NON-NLS-1$
		} catch (PartInitException ex) {
			Activator.log.error("Unable to open editor", ex); //$NON-NLS-1$
		}
		return true;
	}
